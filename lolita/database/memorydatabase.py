...
channeldb = mongo_database.cplaymode
commanddb = mongo_database.commands
cleandb = mongo_database.cleanmode
playmodedb = mongo_database.playmode
playtypedb = mongo_database.playtypedb
authdb = mongo_database.adminauth
videodb = mongo_database.videocalls
onoffdb = mongo_database.onoffper
suggdb = mongo_database.suggestion
autoenddb = mongo_database.autoend
...
async def get_vid_bit_name(chat_id: int) -> str:
    ...
    if not mode:
        if config.PrivateMode == str(True):
            return "High"
        else:
            return "Medium"
